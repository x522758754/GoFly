...
namespace NetWork
{
    public class TcpClientWorker
    {
        ...
        /// 异步发送标记
        /// </summary>
        private bool m_asyncSend = false;
        /// <summary>
        /// 网络通信同步锁
        /// </summary>
        ...
        private void DoReceive()
        {
            ...
            int recvBufferRead = 0;//recvBuffer已经读取的字节数
            ...
            while (m_threadRunFlag)
            {
                do
                {
                    try
                    {
                        recvBufferFree = c_MaxBufferSize - recvBufferRead;
                        if (recvBufferFree > 0)
                        {
                            byteRead = m_socket.Receive(m_recvBuffer, recvBufferRead, recvBufferFree, SocketFlags.None);
                            recvBufferRead += byteRead;
                    ...
                    SplitPackets(recvBufferRead);
                }
                while (byteRead > 0) ;
            }
        }

        private void SplitPackets(int nRecvBufferRead)
        {
            try
            {
                int offset = 0;
                while (nRecvBufferRead > NetEnCoder.GetIntLength())
                {
                    try
                    {
                        //备注：recvBuffer长度=长度（nCode和body所占字节长度） + nCode + body
                        int nLength = NetEnCoder.Decode(m_recvBuffer, ref offset);
                        nRecvBufferRead -= offset;

                        if (nRecvBufferRead >= nLength)
                        {
                            int nCode = NetEnCoder.Decode(m_recvBuffer, ref offset);
                            int nCount = nLength - offset;
                            object msg = NetEnCoder.Decode(nCode, m_recvBuffer, offset, nCount);
                        ...
                        Buffer.BlockCopy(m_recvBuffer, offset, m_recvBuffer, 0, nRecvBufferRead);
                    }
                    catch (Exception e) 
                    {
                        LoggerHelper.Except(e);
            ...
            lock (m_sendQueueLocker)
            {
                while (nTotalLength < c_MaxBufferSize && m_sendQueue.Count > 0)
                {
                    ...
                    if (nTotalLength + bytes.Length < c_MaxBufferSize)
                    {
                        ...
                        Buffer.BlockCopy(bytes, 0, m_sendBuffer, 0, nTotalLength);
                        m_sendQueue.Dequeue();
                    }
                    else
                    {
