Shader "Fairy Tails/MobileUIBlur" {
	Properties {
		[Toggle(ENABLE_Luminance)] _ENABLE_Luminance("_ENABLE_Luminance", Float) = 0
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_Size("Blur", Range(0, 8)) =1
		_bright("Bright",Range(0.1, 0.9)) = 0.6
	}
...
#include "UnityCG.cginc"
#pragma shader_feature ENABLE_Luminance
		  struct appdata_t {
...
#if ENABLE_Luminance
		 fixed grayscale = Luminance(sum.rgb);
		 sum.rgb = grayscale.xxx;
#endif
		 sum.a = 1;
		 return sum;
	  }
