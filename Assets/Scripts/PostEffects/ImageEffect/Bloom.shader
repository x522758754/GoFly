...
Shader "Fairy Tails/MobileBloom" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_Bloom12 ("Bloom (RGB)", 2D) = "black" {}
	}
	...
	CGINCLUDE
		...
		{	
			...
			return color + tex2D(_Bloom12, i.uv.zw);
		} 
		...
		{	
			...
			//color *= max(rq, br - 0.2) / max(br, 1e-5);
