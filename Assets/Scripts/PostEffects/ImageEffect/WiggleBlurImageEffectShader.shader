Shader "Fairy Tails/WiggleBlurImageEffect"
{
	Properties
	{
		...
		_ParamsWiggle("Frequency (X) Amplitude (Y) Timer (Z)", Vector) = (0, 0, 0, 0)
	}
	...
	{
		half2 t = i.uv;
		t.x += sin(_ParamsWiggle.z + t.x * _ParamsWiggle.x) * _ParamsWiggle.y;
		t.y += cos(_ParamsWiggle.z + t.y * _ParamsWiggle.x) * _ParamsWiggle.y - _ParamsWiggle.y;
